...
namespace SimpleWebBrowser
{
        ...
        IPointerUpHandler
    {
        ...
        public delegate void ShowDialoge(string message, string prompt, DialogEventType type);
        public event ShowDialoge OnShowDialog;
        ...
        public Vector2 GetScreenCoords()
        {
            var _raycaster = GetComponentInParent<GraphicRaycaster>();
            var _input = FindObjectOfType<StandaloneInputModule>();
            return GetScreenCoords(_raycaster, _input); 
        }

        private Vector2 GetScreenCoords(GraphicRaycaster ray, StandaloneInputModule input)
        {
            var mousepos = Input.mousePosition;
            var relativeToDisplayPosition = UnityEngine.Display.RelativeMouseAt(mousepos);
        ...
        void Update()
        {
            ...
            //for some reason c# doesn't get the text when using JSQuery but does when using  a dialog (e.g. alert(), promt(), confirm()).
            //Thus the Normal Dialog Box is deactivated and the Dialog is instead used for Communication from javascript to c#
            //If you want to reverse this just uncommment ShowDialog();
            //Dialog
            if (_showDialog)
            {
                _showDialog = false;
                //ShowDialog();
                _mainEngine.SendDialogResponse(true, _dialogMessage);
                if (OnShowDialog != null)
                    OnShowDialog(_dialogMessage, _dialogPrompt, _dialogEventType);
            }
